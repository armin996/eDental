...
class PaymentService extends BaseProvider<Payment, PaymentSearchRequestDto> {
  ...
  PaymentService(
    ...
    super.url = dotenv.env["PAYMENTAPI_URL"];
  }

  Future<List<Payment>> getPaymentsForUser() async {
    if (payments.isEmpty) {
      PaymentSearchRequestDto sRequest = PaymentSearchRequestDto(null, userId);
  ...
  Future<String> payTreatment(User user, int treatmentId, int appointmentId,
      double price, String treatmentName) async {
    ...
    if (paymentIntentResult != 'Failure' ||
      ...
      Payment paymentCreateRequest = Payment(
          DateTime.now(), price, userId, treatmentId, appointmentId,
  ...
  Future<PaymentIntent?> confirmPayment(String clientName,
