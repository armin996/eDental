...
namespace DentalOffice.Repositories.Services
{
    public class AppointmentRepository : BaseRepository<int, AppointmentDto, AppointmentSearchRequestDto, Appointment>
    {
        ...
        public override async Task<List<AppointmentDto>> GetAll(AppointmentSearchRequestDto? searchRequest)
        {
            ...
            {
                searchRequest.SearchTerm = searchRequest.SearchTerm.ToLower();
                appointments = appointments.Where(a => a.ClientFullName.ToLower().Contains(searchRequest.SearchTerm) ||
                                                        a.TreatmentName.ToLower().Contains(searchRequest.SearchTerm) ||
                                                        a.DentistFullName.ToLower().Contains(searchRequest.SearchTerm));
            }
            else
            {
